<template>
  <div>
    ...
    <div ref="main" id="main" style="width: 1000px;height:500px;"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getServerDate() {
      axios
        .get('./php/select/select_mapDateTable.php?name=month&date='+this.dateMonth)
        .then((res) => {
          this.insert = res.data.newInsert;
          this.complete = res.data.newComplete;
          console.log(this.insert)
          for (let i = 1; i <=this.insert.length ; i++) {
            this.allData[i-1] = i+'日'
          }
